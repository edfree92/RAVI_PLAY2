...
const ADVENTURE_MUSIC = {
  ...
  SENSIBLE: "./BASE/SENSIBLE.AUDIO.mp3",
  ECOMMERCE:"./BASE/ECOMMERCE.AUDIO.mp3"
};
...
let   _pendingPlayerB   = "";
let   _rouletteTicker   = null; // intervalo activo del slot machine
...
function openRouletteModal() {
  ...
  const playBtnReset = document.getElementById("roulettePlayBtn");
  playBtnReset.style.display = "inline-block";
  playBtnReset.disabled      = false;   // ← resetear disabled de la ruleta anterior
...
function _spinRoulette() {
  // Cancelar cualquier animación previa antes de empezar
  if (_rouletteTicker) { clearInterval(_rouletteTicker); _rouletteTicker = null; }
  ...
  // ticker ahora es _rouletteTicker (variable de módulo)
  ...
  function tick() {
    ...
    clearInterval(_rouletteTicker);
    _rouletteTicker = setInterval(tick, interval);
  }
