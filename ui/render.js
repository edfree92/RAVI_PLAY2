...
// Primera viñeta de aventura — sin ATRÁS (no hay escena anterior)
function renderNarrativeStart() {
  stage.appendChild(createButton("SIGUIENTE", "next", nextScene));
}

// Viñeta final con menú — ATRÁS + VER PUNTUACIÓN + REGRESAR AL MENÚ
function renderFinalMenu() {
  stage.appendChild(createButton("ATRÁS",           "back",   prevScene));
  stage.appendChild(createButton("VER PUNTUACIÓN",  "action", showRanking));
  stage.appendChild(createButton("REGRESAR AL MENÚ","final",  () => { finishRun(); }));
}
